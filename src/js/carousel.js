(function($) {
  ...
  class Carousel extends Component {
    ...
    constructor(el, options) {
      ...
      // Setup
      this.hasMultipleSlides = this.$el.find('.carousel__item').length > 1;
      ...
      if (this.options.fullWidth) {
        ...
        if (this.showIndicators) {
          this.$el.find('.carousel__fixed-item').addClass('with-indicators');
        }
      }

      // Iterate through slides
      this.$indicators = $('<ul class="carousel__indicators"></ul>');
      this.$el.find('.carousel__item').each((el, i) => {
        this.images.push(el);
        if (this.showIndicators) {
          let $indicator = $('<li class="indicator__item"></li>');
    ...
    _handleResize(e) {
      if (this.options.fullWidth) {
        this.itemWidth = this.$el
          ...
          .first()
          .innerWidth();
        this.imageHeight = this.$el.find('.carousel__item.active').height();
    ...
    _setCarouselHeight(imageOnly) {
      let firstSlide = this.$el.find('.carousel__item.active').length
        ? this.$el.find('.carousel__item.active').first()
        : this.$el.find('.carousel__item').first();
      let firstImage = firstSlide.find('.carousel__img').first();
    ...
    _scroll(x) {
      ...
      if (!this.$el.hasClass('carousel--scrolling')) {
        this.el.classList.add('carousel--scrolling');
      }
      if (this.scrollingTimeout != null) {
        window.clearTimeout(this.scrollingTimeout);
      }
      this.scrollingTimeout = window.setTimeout(() => {
        this.$el.removeClass('carousel--scrolling');
      ...
      if (this.showIndicators) {
        ...
        let activeIndicator = this.$indicators.find('.indicator__item.active');
        if (activeIndicator.index() !== diff) {
          activeIndicator.removeClass('active');
          this.$indicators
            .find('.indicator__item')
            .eq(diff)[0]
      ...
      if (!this.noWrap || (this.center >= 0 && this.center < this.count)) {
        ...
        if (!$(el).hasClass('active')) {
          this.$el.find('.carousel__item').removeClass('active');
      ...
      // onCycleTo callback
      let $currItem = this.$el.find('.carousel__item').eq(this._wrap(this.center));
