(function($, anim) {
  ...
  class Dropdown extends Component {
    ...
    _handleMouseLeave(e) {
      ...
      let leaveToDropdownContent = !!$(toEl).closest('.dropdown__content').length;
      let leaveToActiveDropdownTrigger = false;

      let $closestTrigger = $(toEl).closest('.dropdown__trigger');
      if (
    ...
    _handleDocumentClick(e) {
      ...
      if (
        ...
        $target.closest('.dropdown__content').length &&
        !this.isTouchMoving
      ) {
        ...
        $target.closest('.dropdown__trigger').length ||
        !$target.closest('.dropdown__content').length
      ) {
        setTimeout(() => {
          this.close();
    ...
    _handleDocumentTouchmove(e) {
      let $target = $(e.target);
      if ($target.closest('.dropdown__content').length) {
    ...
    _handleDropdownClick(e) {
      ...
      if (typeof this.options.onItemClick === 'function') {
        let itemEl = $(e.target).closest('.dropdown__item')[0];
